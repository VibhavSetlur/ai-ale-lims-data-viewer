...
const sqlite = option("--sqlite"), manifestPath = option("--manifest"), database = option("--database"), testFixturePath = option("--test-fixture");
...
if (testFixturePath && process.env.NODE_ENV !== "test") throw new Error("--test-fixture is available only when NODE_ENV=test.");
const manifest = JSON.parse(readFileSync(manifestPath, "utf8")); const testFixture = testFixturePath ? JSON.parse(readFileSync(testFixturePath, "utf8")) : undefined;
if ((!testFixture && manifest.sourceSha256 !== APPROVED_SOURCE_SHA256) || !Array.isArray(manifest.legacyInventory) || !manifest.inventorySha256) throw new Error("Staging requires an inspection manifest for the approved source with its frozen legacy inventory.");
if (testFixture && manifest.sourceSha256 !== testFixture.sourceSha256) throw new Error("Test fixture source checksum does not match the inspection manifest.");
const actual = testFixture ? inspectSqliteFixture(sqlite, testFixture) : inspectSqlite(sqlite, APPROVED_SOURCE_SHA256);
