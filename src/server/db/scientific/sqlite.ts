...
export class SqliteScientificRepository implements ScientificRepository {
  ...
  private analysis(query: MutationReadRequest, kind: "mutations" | "growth" | "library" | "copy"): AnalysisResult {
    ...
    if (kind === "growth" && query.registryKey) throw new AppError("SEMANTIC_INVALID", "Growth records cannot be scoped truthfully to the requested registry.");
    this.validateAnalysisScope(query); const warnings: string[] = []; const caps = this.capabilities(); const placeholders = query.sampleKeys.map(() => "?").join(","); let rows: Record<string, unknown>[] = [];
