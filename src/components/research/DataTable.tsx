...
import { IS_STATIC, fetchData } from '@/lib/dataSource';
...
export default function DataTable({ tableName }: DataTableProps) {
    ...
    (async () => {
      ...
      try {
        if (IS_STATIC) {
          ...
          const queryTableStatic = (_args: unknown): Promise<{ schema: ColumnSchema[]; rows: Record<string, unknown>[]; totalCount: number; totalPages: number }> => { throw new Error('Static mode unsupported'); };
        ...
        const res = await fetchData(`/api/data/${encodeURIComponent(tableName)}?${params.toString()}`);
        if (cancelled) return;
  ...
  const loadDistinct = useCallback(async (column: string) => {
    ...
    try {
      if (IS_STATIC) {
        const distinctStatic = (_t: string, _c: string, _n: number): Promise<string[]> => { throw new Error('Static mode unsupported'); };
        const values = await distinctStatic(tableName, column, 200);
        setDistinctCache(p => ({ ...p, [column]: { values, truncated: values.length >= 200 } }));
        return;
      }
      const res = await fetchData(`/api/distinct/${encodeURIComponent(tableName)}?column=${encodeURIComponent(column)}&limit=200`);
      const json = await res.json();
      if (!res.ok) return;
  ...
  const exportCsv = async () => {
    ...
    try {
      ...
      if (IS_STATIC) {
        ...
        const queryTableStatic = (_args: unknown): Promise<{ schema: { name: string }[]; rows: Record<string, unknown>[] }> => { throw new Error('Static mode unsupported'); };
