...
import type { RouteView } from '@/lib/routes';
...
export default async function ViewerPage({ view }: { view: RouteView }) {
  const tables = await getInitialTables();
  // Counts are populated by the client after mount (via the data source), which keeps
  // initial render fast on large DBs.
  const buildInfo = getBuildInfo();
  return (
    <main className="h-screen w-full flex flex-col overflow-hidden">
      <Dashboard initialTables={tables} buildInfo={buildInfo} view={view} />
    </main>
  );
}
