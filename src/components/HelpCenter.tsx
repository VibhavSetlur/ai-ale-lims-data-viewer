...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: 'steps', items: [
        ...
        'Export the visible heatmap as PNG, SVG, HTML, or print to PDF; export CSV for the underlying values.',
      ] },
  ...
  },
  {
    ...
    summary: 'Every visualization exports as PNG, SVG, HTML, or print-to-PDF; CSV for quantitative values.',
    blocks: [
      { kind: 'p', text: 'Each visualization has an Export figure button that opens a small menu with four options. Every export is rendered as a self-contained, publication-quality figure on a white background with a title and a snapshot caption baked in (it is NOT a screenshot of your screen): chart colors and fonts are resolved to fixed values so the figure looks identical wherever you open it. Use CSV (separate button) whenever you will make a quantitative claim.' },
      { kind: 'bullets', items: [
        'PNG: a high-resolution 3x bitmap, best for slides, email, and quick sharing.',
        'SVG: a fully self-contained vector; scales crisply to any size and stays editable in Illustrator/Inkscape for manuscripts. This is the recommended format for figures.',
        'HTML: a self-contained page where labels keep their exact pixel size (best for the barcode Rows view).',
        'Print / Save PDF: opens the print dialog scoped to just the figure.',
      ] },
      { kind: 'note', text: 'Whatever is on screen is what exports. Isolate a lineage, set the filters, or pick Reads vs Fraction first, then export, so the figure shows exactly the comparison you mean. Scrollable views (the Comparative heatmap) export in full, not just the visible part.' },
  ...
  },
  {
    ...
    blocks: [
      { kind: 'bullets', items: [
        ...
        'An export looks low-resolution: PNG is a high-res 3x bitmap (great for slides); for manuscripts use SVG, which is infinite-resolution vector and editable.',
      ] },
