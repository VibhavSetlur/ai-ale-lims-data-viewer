...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    icon: <Layers className="w-4 h-4" />,
    summary: 'Add charts to Compare for honest small-multiples: shared Y-axis, shared legend, synced selection and hover.',
    blocks: [
      { kind: 'p', text: 'A comparison is only honest if the axes are shared, and only useful if selection and hover sync across panels.' },
      { kind: 'bullets', items: [
        'Common Y-axis: every compared chart uses one read-count scale, so equal bar heights mean equal reads.',
        'Shared candidate legend: clicking a candidate toggles it in every chart at once; hovering highlights it across all charts.',
        'Column-count control (Auto/1/2/3/4) and Clear all keep the layout under your control.',
        'Selection in the left sidebar and selection in the shared legend are the same selection — they never disagree.',
      ] },
    ],
  },
  {
  ...
  },
  {
    ...
    blocks: [
      { kind: 'p', text: 'Each visualization has an Export figure button that opens a small menu with four options. Use CSV (separate button) whenever you will make a quantitative claim.' },
      { kind: 'bullets', items: [
        'PNG: a 2x bitmap, best for slides, email, and quick sharing.',
        'SVG: the vector source of an SVG chart; scales crisply and stays editable for manuscripts.',
  ...
  },
  {
    ...
    blocks: [
      { kind: 'bullets', items: [
        ...
        'An export looks low-resolution: use SVG (vector) for manuscripts; PNG is a 2x bitmap meant for slides and email.',
      ] },
