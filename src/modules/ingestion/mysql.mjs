...
export function mysqlUrlFromSecret({ file, stdin = false, purpose }) {
  ...
  let text;
  if (stdin) text = readFileSync(0, "utf8"); else { if ((statSync(file).mode & 0o077) !== 0) throw new Error("MySQL secrets file must not be accessible by group or others."); text = readFileSync(file, "utf8"); }
  let secret; try { secret = JSON.parse(text); } catch { throw new Error("MySQL secret input must be JSON."); }
  const value = secret[purpose]; if (typeof value !== "string" || !value.startsWith("mysql")) throw new Error(`MySQL secret input is missing ${purpose}.`); return value;
}
export async function connect(url, database) { quoteIdentifier(database); const connection = await mysql.createConnection(url); await connection.query(`USE ${quoteIdentifier(database)}`); const [rows] = await connection.query("SELECT DATABASE() AS name"); if (String(rows[0]?.name ?? "") !== database) { await connection.end(); throw new Error("MySQL connection database does not match requested database."); } return connection; }
export function mysqlType(sqliteType) { const type = String(sqliteType ?? "").toUpperCase(); if (type.includes("INT")) return "BIGINT"; if (type.includes("REAL") || type.includes("FLOA") || type.includes("DOUB")) return "DOUBLE"; if (type.includes("BLOB")) return "LONGBLOB"; if (type.includes("NUM") || type.includes("DEC")) return "DECIMAL(65,30)"; if (type.includes("CHAR") || type.includes("CLOB") || type.includes("TEXT") || type === "") return "LONGTEXT"; throw new Error(`Unsupported SQLite affinity: ${sqliteType}`); }
export function createTableSql(table) { const columns = table.columns.map((column) => `${quoteIdentifier(column.name)} ${mysqlType(column.type)} ${column.notnull ? "NOT NULL" : "NULL"}${column.dflt_value == null ? "" : ` DEFAULT ${column.dflt_value}`}`); const keys = table.columns.filter((column) => Number(column.pk) > 0).sort((a, b) => Number(a.pk) - Number(b.pk)).map((column) => quoteIdentifier(column.name)); if (keys.length) columns.push(`PRIMARY KEY (${keys.join(", ")})`); return `CREATE TABLE ${quoteIdentifier(table.name)} (${columns.join(", ")}) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_as_cs`; }
export function createIndexSql(table, index) { if (index.origin === "pk") return null; if (!index.columns.length) throw new Error(`Index ${index.name} has no columns.`); return `CREATE ${index.unique ? "UNIQUE " : ""}INDEX ${quoteIdentifier(index.name)} ON ${quoteIdentifier(table.name)} (${index.columns.map((column) => `${quoteIdentifier(column.name)} ${column.desc ? "DESC" : "ASC"}`).join(", ")})`; }
export async function createMetadata(connection) {
  await connection.query("CREATE TABLE IF NOT EXISTS scientific_publication_event (event_id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY, candidate_database VARCHAR(64) NOT NULL, source_sha256 CHAR(64) NOT NULL, manifest_digest CHAR(64) NOT NULL, reconciliation_digest CHAR(64) NOT NULL, published_at DATETIME(6) NOT NULL, UNIQUE KEY scientific_publication_event_candidate (candidate_database, reconciliation_digest)) ENGINE=InnoDB");
