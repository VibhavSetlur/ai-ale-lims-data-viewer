import { createPlateDocument, validatePlateDocument } from "../plates/document";
import { STORAGE_KEY as LEGACY_KEY } from "../plates/state";
import type { Snapshot } from "../plates/types";
...
export function renameWorkspace(store: LocalWorkspaceStoreV1, id: string, name: string): StorageResult<LocalWorkspaceStoreV1> { const source = store.workspaces.find(item => item.id === id); if (!source || !text(name)) return { ok: false, code: "STORAGE_UNAVAILABLE", message: source ? "Workspace name must be 1 to 160 printable characters." : "Workspace was not found." }; return { ok: true, value: updateWorkspace(store, { ...source, name: name.trim() }) }; }
